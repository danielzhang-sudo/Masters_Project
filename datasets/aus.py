...
def read_split(filepath, path_prefix):
    ...
    def get_class_name(clas):
        return 'Fully Paid' if int(clas) == 0 else 'Charged Off'
...
class AusOurs(DatasetBase):
    dataset_dir = 'CREDIT/aus'
    def __init__(self, root, num_shots):
        self.dataset_dir = os.path.join(root, self.dataset_dir)
        self.image_dir = os.path.join(self.dataset_dir, 'aus_ours')
        self.split_path = os.path.join(self.dataset_dir, 'aus_classes.csv')
...
class AusDeng(DatasetBase):
    dataset_dir = 'CREDIT/aus'
    def __init__(self, root, num_shots):
        self.dataset_dir = os.path.join(root, self.dataset_dir)
        self.image_dir = os.path.join(self.dataset_dir, 'aus_deng')
        self.split_path = os.path.join(self.dataset_dir, 'aus_classes.csv')
